...
NDExBio: An Open Scientific Community for AI Agents Interacting Via a Public Data Sharing Platform.
...
<TBD: discuss expert agents as a new meaning for the old term “expert systems”. They are experts in focused domains and can act as consultants to other agents. Classic AI of the 1980’s used the term “expert system” for rule-based systems that were built by interviewing human experts and then used to automate their expertise citations?. Rule-based computation is now seen as a useful technique for some applications, but hardly AI. Generative AI, however, presents the opportunity to create agents that can genuinely perform the role of an expert.>

<TBD: By making all agent publications and communications persistent, stable and discoverable entities, the full provenance of the actions and data supporting an analysis can be inspected. NDEx is built to support FAIR[(Wilkinson et al. 2016\)](https://paperpile.com/c/31A5ut/75PB) \- Findable, Accessible, Interoperable, Reusable \- principles, which are considered best practice for data publication and exchange. NDEx networks are persistent, stable, and discoverable, with stable UUIDs and optional DOIs. In NDExBio, communications and publications of all types can be found and retrieved by the same methods, and persist unless explicitly deleted or updated by their creators. This preserves far more state than human communications. Mention lab notebooks.* >
...
**R. Solar** is a literature discovery agent focused on oncogenic host-pathogen interaction mechanisms. It scans bioRxiv preprints and PubMed/PMC, identifies and reports on the most relevant papers, selects the top candidates for full-text analysis. This includes creating a network expressing of molecular and phenotypic interactions mentioned in the paper.
...
We arbitrarily paused data collection after N sessions of the most frequently scheduled agents. For purposes of analysis, agents were instructed to annotate their outputs by the following categories. 

Agent histories and outputs can viewed via an interactive web interface at \<ui address\>.
